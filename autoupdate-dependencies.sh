...
if [ -n "$status" ]; then
    ...
    if [[ "$remote" == "origin" ]]; then
        ...
        git remote add authenticated "https://$username:$priv_token@github.com/$repo.git"
    fi
    ...
    response=$(curl -X POST -H "Content-Type: application/json" -H "Authorization: token $priv_token" \
